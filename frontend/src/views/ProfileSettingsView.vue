<template>
  <section class="profiles-page">
    <div class="page-heading"><div><p class="page-eyebrow">管理员配置</p><h2>配置档案管理</h2><p>维护已批准的模型和知识库连接；技术参数只在管理员工作区显示。</p></div><el-button type="primary" @click="openCreate"><Plus :size="15" />新增配置档案</el-button></div>
    ...
    <div class="profile-tabs"><button :class="{ active: kind === 'model' }" type="button" @click="kind = 'model'">模型配置档案 <span>{{ modelProfiles.length }}</span></button><button :class="{ active: kind === 'knowledge' }" type="button" @click="kind = 'knowledge'">知识库配置档案 <span>{{ knowledgeProfiles.length }}</span></button></div>
    <div v-if="loading" class="loading-line">正在读取配置档案...</div>
    <div v-else class="profile-table-wrap"><table class="profile-table"><thead><tr><th>名称</th><th>医学侧选项</th><th>技术配置摘要</th><th>状态</th><th></th></tr></thead><tbody><tr v-for="profile in activeProfiles" :key="profile.id"><td><strong>{{ profile.name }}</strong><span>{{ profile.description || '暂无说明' }}</span></td><td><code>{{ compactJson(profile.medical_options) }}</code></td><td><code>{{ compactJson('technical_config' in profile ? profile.technical_config : {}) }}</code></td><td><span class="active-pill" :class="{ muted: 'is_active' in profile && !profile.is_active }">{{ 'is_active' in profile && !profile.is_active ? '停用' : '启用' }}</span></td><td><button class="icon-action" title="编辑配置档案" type="button" @click="openEdit(profile)"><Pencil :size="15" /><span class="sr-only">编辑</span></button></td></tr></tbody></table><div v-if="!activeProfiles.length" class="empty-state">还没有{{ kind === 'model' ? '模型' : '知识库' }}配置档案。</div></div>

    <el-dialog v-model="dialogVisible" :title="editing ? '编辑配置档案' : '新增配置档案'" width="min(620px, calc(100vw - 32px))"><el-form :model="form" label-position="top"><el-form-item label="类型"><el-radio-group v-model="kind" :disabled="Boolean(editing)"><el-radio-button label="model">模型</el-radio-button><el-radio-button label="knowledge">知识库</el-radio-button></el-radio-group></el-form-item><el-form-item label="名称" required><el-input v-model="form.name" /></el-form-item><el-form-item label="说明"><el-input v-model="form.description" /></el-form-item><el-form-item label="对医学用户开放"><el-switch v-model="form.exposed_to_medical" /></el-form-item><el-form-item label="医学语义选项 JSON"><el-input v-model="form.medical_options" :rows="4" type="textarea" /></el-form-item><el-form-item label="技术配置 JSON"><el-input v-model="form.technical_config" :rows="7" type="textarea" /></el-form-item><p v-if="formError" class="form-error">{{ formError }}</p></el-form><template #footer><el-button @click="dialogVisible = false">取消</el-button><el-button :loading="saving" type="primary" @click="save">保存</el-button></template></el-dialog>
  </section>
</template>
...
async function save() {
  formError.value = ''
  if (!form.name.trim()) { formError.value = '请输入配置档案名称。'; return }
...
@media (max-width: 700px) {
  ...
  .page-heading :deep(.el-button) { width: 100%; margin-top: 15px; }
  .profile-table-wrap { box-shadow: none; }
  .profile-table { min-width: 740px; }
}
</style>

...
@media (max-width: 980px) { .compare-grid { grid-template-columns: 1fr; }.run-config { grid-template-columns: 1fr 1fr; }.run-config > :last-child { grid-column: 1 / -1; } }.run-config :deep(.el-radio-group) { display: flex; }.run-config :deep(.el-radio-button__inner) { padding: 8px 12px; }
@media (max-width: 620px) { .test-heading { display: block; }.run-actions { margin-top: 15px; }.run-actions :deep(.el-button) { flex: 1; }.run-config { grid-template-columns: 1fr; }.run-config > :last-child { grid-column: auto; } }
</style>

...
输入节点从提取上下文选择字段，支持重命名、默认值和输出 Schema，输出只读对象供后续节点引用。

为适配决策树中“获取住院文书、检验、检查、医嘱并形成基线资料/治疗背景”等需求，增加医学数据准备节点。医学用户可从原始 JSON、提取数据和任意上游节点中勾选字段，按“病理信息、基线资料、治疗背景、生育相关、脑转移相关”等业务分组组织输出。节点支持必填/可选字段、数组筛选、时间范围、取最新/首次记录、类型转换、字段别名、缺失项清单和数据充分性状态。

字段选择界面使用中文路径树和样例值，不要求用户编写 JSONPath。平台在底层保存稳定路径，并在源数据结构变化时标记失效字段和替代候选。
...
- 知识库参数：查询模板、指南/版本范围、top_k、BM25 开关、引用是否必填和证据不足策略。
...
正式平台不提供 `.pos` 导入。参考决策树中的图形语义由平台原生节点覆盖：判定图元使用条件/临床决策节点，资料获取与补齐使用医学数据准备或临床任务节点，大模型判断与方案生成使用模板化 LLM 节点，MDT 页面引用使用子工作流/MDT 节点，结束和路径跳转使用输出节点，科室和备注使用说明/协同节点。
...
后端单元测试覆盖 JSON 提取、版本不可变性、普通环路校验、受控重评回环、条件/多出口判定、Python 沙箱、模型请求转换、知识库响应转换和证据链接。集成测试使用 Mock Chat Completions 服务和 Mock Knowledgebase 服务验证完整运行链路。增加由平台原生节点定义的 HER2 参考工作流 fixture，验证字段筛选、决策节点、分支标签、MDT 引用、提示词参数、未连接端点和重评回环。前端测试覆盖五个核心工作区、医学侧表单编辑、复制/粘贴规则、证据抽屉和原文打开；端到端测试覆盖“从参考模板创建工作流 → 医学人员修订字段/分支/提示词 → 测试 → 点击原文引用 → 发布版本 → 运行追踪”。
